...
#define SDP_BIT_MASK (((1ULL << (VA_SIZE - N_SDP_BITS_COMPLEMENT)) - 1ULL) << PDP_STARTING_BIT)
#define GET_SDP_BITS(va) ((va & (SDP_BIT_MASK)))

// PDP bits are the next 9 ([30:38])
#define PDP_BIT_MASK (((1ULL << (((VA_SIZE - (N_SDP_BITS_COMPLEMENT + N_PDP_BITS_COMPLEMENT))) - 1ULL)) << PDP_STARTING_BIT))
...
#define PDE_BIT_MASK (((1ULL << (VA_SIZE - (N_PDP_BITS_COMPLEMENT + N_PDP_BITS_COMPLEMENT + N_BITS_PDE_COMPLEMENT))) - 1ULL) << PDE_STARTING_BIT)
#define GET_PDE_BITS(va) ((va & (PDE_BIT_MASK)))
#define GET_PDE_OFFSET(va) (va & ~PDE_BIT_MASK)

// PTE bits are the middle 9 below PDE ([12:20])
#define PTE_BIT_MASK (((1ULL << (VA_SIZE - (N_PDP_BITS_COMPLEMENT + N_PDP_BITS_COMPLEMENT + N_BITS_PDE_COMPLEMENT + N_BITS_PTE_COMPLEMENT))) - 1ULL) << PTE_STARTING_BIT)
