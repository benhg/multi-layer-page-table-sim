...
typedef struct page_table_entry {
    ...
    union {
        uintptr_t oneg_pte_index : 8;   //< One gig PTE offset (which page entry do we read)
    ...
    struct {
        uint32_t pid;                 //< PID the page is assigned to
        							  // Also not stored in hardware. PID enforcement is done by the fact that each process has its own page table.
