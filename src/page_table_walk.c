...
uintptr_t walk(address_context_t * a_ctx, ptw_sim_context_t *ctx){
	...
	// If the page is labelled as a 1G page, that means we found our page and we should do the translation
	if (pdp->page_metadata.page_size == ONE_G){

		// Check for permissions
		if (!check_permissions(a_ctx->permissions, pdp->page_metadata.permissions)){
			return -EUNAUTHORIZED;
		}
	...
	// At this point, all 3 page sizes are valid, the only difference is what macro we use. So we can pull out the checks and run them up front.
	if (pte->page_metadata.page_size == ONE_G){
